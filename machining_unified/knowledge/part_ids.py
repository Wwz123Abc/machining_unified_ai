...
#
# 中段允许字母：IMU 系列用 IMU180-22A-000、IMU180-A01-000、IMU180-B30-000 这类
# 分区编号，若限定三位数字会漏掉 11/17 个装配号。实测放宽后，现有 66 条企业证据
# 与 24 条 CAD 记录里只有 TEACH-CAD-001/002/003 的识别结果发生变化，
# 而它们本就是零件编号——是补上漏识别，不是引入误命中。
PART_ID_PATTERN = re.compile(r"(?:[0-9]{3})?[A-Z]{2,6}[0-9]{0,3}-[0-9A-Z]{3}-[0-9]{3}[A-Z]?")
